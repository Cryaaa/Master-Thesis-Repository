...
def workflow(gpu_input, number_of_dilations = 15, number_of_erosions = 9):
    import numpy as np
    import pyclesperanto_prototype as cle    
    ...
    #labels3d = only3dlabels(alllabels)
    #alllabels = None
...
def only3dlabels(gpu_label_image):
    import pyclesperanto_prototype as cle
    import numpy as np
    
    cleregionprops = cle.statistics_of_background_and_labelled_pixels(gpu_label_image)
...
cc_dataset = bs.ClearControlDataset('F://2019-10-28-17-22-59-23-Finsterwalde_Tribolium_nGFP')
...
for i in gastrulation_times:
    tribolium = cc_dataset.get_image(time_in_seconds=i)[16:,60:,:]
    ...
    #propscombined.to_csv(savedata_location + 'Finsterwalde Gastrulation Labels//complete regionprops of t = {}s.csv'.format(i))
