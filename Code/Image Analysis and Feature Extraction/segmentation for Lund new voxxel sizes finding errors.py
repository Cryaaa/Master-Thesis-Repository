...
def workflow(image, number_of_dilations = 15, number_of_erosions = 9):
    ...
    gpu_labelled_spots = cle.connected_components_labeling_diamond(gpu_masked_spots)
...
def only3dlabels_modified(gpu_label_image,original_image):
    import pyclesperanto_prototype as cle
    import numpy as np
    
    from skimage.measure import regionprops_table
    regionprops = regionprops_table(cle.pull(gpu_label_image).astype(dtype = 'uint16'), properties=('area','bbox'))

    
    bboxdiffx = regionprops['bbox-0'] - regionprops['bbox-3']
    bboxdiffy = regionprops['bbox-1'] - regionprops['bbox-4']
    bboxdiffz = regionprops['bbox-2'] - regionprops['bbox-5']
    
    flaglist =[0]
    for i in range(int(len(bboxdiffx))):
        if (bboxdiffx[i] <=1 or bboxdiffy[i]<=1 or bboxdiffz[i] <= 1):
            flaglist.append(1)
        else:
            flaglist.append(0)
...
# checking 14 and 20
for i in [14,20]:
    rescaled = resample_isotropic_nodownsample(alltrib[i], voxel_size)
...
# finding out whats wrong with 14
rescaled_14 = resample_isotropic_nodownsample(alltrib[14], voxel_size)
output_14, gpu_output = workflow(rescaled_14)

from skimage.measure import regionprops_table
bbox_14 = regionprops_table(output_14.astype(dtype = 'uint16'), properties=('area','bbox'))
