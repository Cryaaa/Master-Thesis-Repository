...
pred_location = 'C:/Users/ryans/OneDrive/Documents/Master Thesis/Documents/Figures/Embryo Serosa Clustering Wolgast//'
regprops_w_predictions = pd.concat([pd.read_csv(pred_location+'regprops with embryo serosa clustering UMAP(nn 400, mindist 0), hdbscan(mincl size 2000) t{}.csv'.format(i))
                                    for i in range(32)],axis = 0)
pred_wolgast = regprops_w_predictions['prediction'].to_numpy()


image_output_folder = 'C:/Users/ryans/OneDrive/Documents/Master Thesis/Documents/Figures/embryo serosa video files//' + 'wolgast/'
rotations_wolgast = [(0,170,0),(0,0,0)]
cmap_napari = tc.napari_label_cmap()

            
for time in range(27,32):
    nice_screenshots_of_1_timepoint(wolgast,pred_wolgast,
                                    time,cmap_napari,image_output_folder, 
                                    'HDBSCAN embryo serosa'
                                    ,rotations_wolgast)  
