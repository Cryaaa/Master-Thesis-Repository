...
lund_embedding = embryo_serosa_preprocessing(complete_regionprops_lund)
...
lund_prediction = clusterer_all.fit_predict(lund_embedding)
np.save('lund embryo serosa embedding.npy',lund_embedding)
np.save('lund embryo serosa presdiction.npy',lund_prediction)


timepoint = 1
label_image = lund.get_labels(timepoint)
intensity_image = lund.get_intensity_image(timepoint)
cum_indices = lund.cumulative_label_lengths()

prediction = lund_prediction[cum_indices[timepoint]:cum_indices[timepoint+1]]
cluster_image = tc.generate_parametric_cluster_image(label_image,cle.push(label_image),prediction)
